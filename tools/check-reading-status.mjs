...
const [status,readerFinished,readerBootstrap,continuousCore,series,library,polish,finishedPolish,style,compactStyle,writeSource,adminBootstrap,adminVersion,headers,pkg,readerHtml,indexHtml,adultHtml]=await Promise.all([
...
for(const marker of ["grid-template-rows:2.5em 1.2em 1.2em","min-height:2.5em!important","grid-row:1","grid-row:2","align-self:end"]){
...
if(!indexHtml.includes("library-compact-alignment.css?v=1.15.8")||!adultHtml.includes("library-compact-alignment.css?v=1.15.8"))fail("Main and Adult Libraries must load the v1.15.8 compact-card alignment stylesheet");
...
for(const marker of ["/data/version.json","/assets/js/reading-status.js","/assets/js/library.js","/assets/js/library-series-polish.js","/assets/js/library-finished-polish.js","/assets/css/library-compact-alignment.css"]){
  if(!headers.includes(marker))fail(`fresh-cache headers are missing ${marker}`);
}
const parsed=JSON.parse(pkg);
if(parsed.version!=="1.15.8")fail("package version must be 1.15.8");
