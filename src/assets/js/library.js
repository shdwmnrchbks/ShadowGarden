...
function queryTokens(value){
  ...
  return matches.map(token=>normalize(token.replace(/^"|"$/g,""))).filter(Boolean);
}
...
function volumeCountMatches(count,range){
...
function seriesHaystack(series){
  return normalize([
    series.title,
    series.author,
    series.description,
...
function card(series,index=0){
  const c=cover(series),vols=arr(series.volumes).length,aboveFold=index<6;
...
function formatDate(value){
  ...
  try{return new Intl.DateTimeFormat(undefined,{month:"short",day:"numeric",year:"numeric"}).format(new Date(time))}catch{return String(value||"")}
}
function recentCard(entry,index){
  const {series,volume}=entry,c=volumeCover(series,volume);
  const title=volume.title||`Volume ${volume.number??"—"}`;
  const badge=volume.number!=null?`VOL ${volume.number}`:"NEW";
  ...
  return `<a class="recent-volume" href="${href}">
    <div class="recent-volume-cover">
      ${c?`<img src="${esc(c)}" alt="${esc(title)} cover" loading="${index<4?"eager":"lazy"}" decoding="async" onerror="this.style.display='none';this.nextElementSibling.classList.remove('hidden')">`:""}
      <div class="recent-volume-fallback ${c?"hidden":""}">${esc(title)}</div>
      <span class="recent-volume-badge">${esc(badge)}</span>
    </div>
    <div class="recent-volume-copy"><strong>${esc(title)}</strong><span>${esc(series.title)} · ${esc(formatDate(volume.added))}</span></div>
  </a>`;
}
function renderRecentlyAdded(){
  const recent=state.items.flatMap(series=>arr(series.volumes).map(volume=>({series,volume,time:addedTime(volume.added)})))
    .filter(entry=>entry.time>0)
    .sort((a,b)=>b.time-a.time)
    .slice(0,8);
  const section=$("#recentSection"),container=$("#recentVolumes");
  if(!section||!container)return;
  if(!recent.length){section.classList.add("hidden");return}
  container.innerHTML=recent.map(recentCard).join("");
...
function collectFilters(){
  ...
  state.items.forEach(series=>new Set(arr(series.tags).map(String)).forEach(tag=>counts.set(tag,(counts.get(tag)||0)+1)));
...
function validateState(){
  const authors=new Set(state.items.map(s=>String(s.author||"").trim()).filter(Boolean));
  const years=new Set(state.items.map(s=>String(s.year||"")).filter(Boolean));
  const tags=new Set(state.items.flatMap(s=>arr(s.tags).map(String)));
  ...
  if(!validVolumeRanges.has(state.volumeRange))state.volumeRange="";
  if(!validSorts.has(state.sort))state.sort="recent";
...
function readUrl(){
  ...
  state.sort=validSorts.has(params.get("sort"))?params.get("sort"):"recent";
...
function writeUrl(mode="replace"){
  ...
  ["q","author","tag","year","vols","sort","pinned","view"].forEach(key=>params.delete(key));
...
function syncControls(){
  ...
  document.querySelectorAll(".view-switch button").forEach(button=>button.classList.toggle("active",button.dataset.view===state.view));
  renderActiveTags();
}
function filterAndSort(){
  ...
  state.filtered=state.items.filter(series=>{
    ...
    if(state.pinnedOnly&&!pins.has(series.id))return false;
    return true;
  });
  state.filtered.sort((a,b)=>{
    ...
    return latest(b)-latest(a)||String(a.title||"").localeCompare(String(b.title||""));
  });
}
function batchSize(){return state.view==="compact"?60:36}
function updateResultCount(){
  const totalSeries=state.filtered.length,totalVolumes=state.filtered.reduce((n,s)=>n+arr(s.volumes).length,0);
  const shown=Math.min(state.renderedCount,totalSeries);
...
function appendBatch(){
  ...
  const html=state.filtered.slice(start,end).map((series,index)=>card(series,start+index)).join("");
  $("#catalogGrid")?.insertAdjacentHTML("beforeend",html);
  state.renderedCount=end;
  updateResultCount();
}
function apply({historyMode=null}={}){
  filterAndSort();
...
function renderContinue(){
  ...
  for(let i=0;i<localStorage.length;i++){
    ...
    try{const item=JSON.parse(localStorage.getItem(key));if(item?.updatedAt)progress.push(item)}catch{}
  }
  progress.sort((a,b)=>b.updatedAt-a.updatedAt);
  const allowedFiles=new Set(state.items.flatMap(series=>arr(series.volumes).map(volume=>volume.file)));
  const saved=progress.find(item=>allowedFiles.has(item.file));
  if(!saved)return;
  const match=state.items.flatMap(series=>arr(series.volumes).map(volume=>({series,volume}))).find(item=>item.volume.file===saved.file);
  if(!match)return;
  $("#continuePanel").innerHTML=`<div class="continue-mark">✦</div><div><strong>${esc(match.volume.title)}</strong><span>${esc(match.series.title)} · ${Math.round((saved.percentage||0)*100)}%</span></div><a href="/reader.html?book=${encodeURIComponent(match.volume.file)}&series=${encodeURIComponent(match.series.id)}">Continue</a>`;
...
function setupIncrementalRendering(){
  ...
  state.observer=new IntersectionObserver(entries=>{
    ...
    requestAnimationFrame(()=>{
      appendBatch();
      setTimeout(()=>{state.autoLoading=false},120);
    });
  },{rootMargin:"800px 0px"});
  state.observer.observe(sentinel);
}
function clearFilters({historyMode="push"}={}){
  state.query="";state.author="";state.tags=new Set();state.year="";state.volumeRange="";state.sort="recent";state.pinnedOnly=false;
  apply({historyMode});
}
...
function bindControls(){
  $("#searchInput")?.addEventListener("input",event=>{
    ...
    searchTimer=setTimeout(()=>apply({historyMode:"replace"}),120);
  });
  ...
  $("#activeTags")?.addEventListener("click",event=>{
    const button=event.target.closest("button[data-remove-tag]");if(!button)return;
    state.tags.delete(button.dataset.removeTag);
    apply({historyMode:"push"});
  });
  ...
  document.querySelector(".view-switch")?.addEventListener("click",event=>{
    ...
    localStorage.setItem(`sg-view:${scope}`,state.view);
    apply({historyMode:"replace"});
  });
  $("#loadMore")?.addEventListener("click",appendBatch);
  $("#recentViewAll")?.addEventListener("click",()=>{
    clearFilters({historyMode:null});
    state.sort="recent";
    apply({historyMode:"push"});
    $("#catalogSection")?.scrollIntoView({behavior:"smooth",block:"start"});
  });
  window.addEventListener("popstate",()=>{
    readUrl();validateState();apply();
  });
}
async function init(){
  setupAdultGate();
  ...
  try{
    ...
    $("#headerVolumes").textContent=state.items.reduce((n,series)=>n+arr(series.volumes).length,0);
    collectFilters();
    validateState();
    renderContinue();
    renderRecentlyAdded();
