...
test("Garden Keeper consumes shared motion without taking workflow ownership",async()=>{
  const [html,app,js,css]=await Promise.all([
    read("src/admin.html"),read("src/assets/js/admin/app.js"),read("src/assets/js/admin/motion.js"),read("src/assets/css/admin-motion.css")
  ]);
  for(const marker of ["/assets/css/motion.css","/assets/css/admin-motion.css"]){
    assert.match(html,new RegExp(marker.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")));
  }
  for(const marker of ["/assets/js/motion.js","/assets/js/admin/motion.js"]){
...
test("Keeper motion assets are architecture-registered and deployment-fresh",async()=>{
  ...
  for(const manifestText of [v1,v2]){
    const manifest=JSON.parse(manifestText),page=manifest.pages.gardenKeeper;
    for(const asset of ["/assets/css/motion.css","/assets/css/admin-motion.css"]){
      assert.ok(page.styles.includes(asset),`Garden Keeper manifest must register first-paint style ${asset}`);
      assert.equal(page.runtimeLoaded.includes(asset),false,`Garden Keeper runtime must not own ${asset}`);
    }
    for(const asset of ["/assets/js/motion.js","/assets/js/admin/motion.js"]){
      assert.ok(page.runtimeLoaded.includes(asset),`Garden Keeper manifest must register runtime ${asset}`);
    }
  }
