...
const [
  ...
  read('docs/releases/v2.6.7.md'),
  read('tests/e2e/specs/continuous-media-width.spec.mjs'),
...
// v2.6 release reconciliation remains synchronized through the v2.6.1–v2.6.7 hotfixes.
assert.equal(rootPkg.version, '2.6.7', 'root package version must be v2.6.7 for the current v2.6 hotfix');
...
assert.match(mediaWidthNotes, /^# Shadow Garden v2\.6\.7 — Continuous Media Width Independence/m);
assert.match(mediaWidthNotes, /text-width/i);
assert.match(mediaWidthNotes, /portrait plates/i);
// v2.6.7: Continuous artwork is independent of the text-width setting.
assert.match(theme, /"max-width": "100vw !important"/, 'Continuous media must expand to the reading canvas instead of the text-width column');
assert.match(theme, /"margin-left": "calc\(\(100% - 100vw\) \/ 2\) !important"/, 'Continuous media bleed must re-center with symmetric negative margins so wide artwork is not right-clipped');
assert.match(theme, /mediaRelease = \{ "max-height": "none !important" \}/, 'Continuous mode must release the epub.js page-column max-height cap on img/svg');
assert.doesNotMatch(theme, /body\["overflow-x"\] = "clip !important"/, 'the body-level overflow-x clip must not come back: it clips the legitimate canvas bleed at the text-width edge');
assert.match(theme, /window\.CSS\.supports\("selector\(div:has\(> img:only-child\)\)"\)/, 'media-only container bleed must be feature-detected for engines without :has()');
assert.match(visualCache, /function syntheticPageCss\(/, 'the synthetic plate stylesheet must be generated by a named owner');
assert.match(visualCache, /html\[data-sg-synthetic-visual="1"\] body\[data-sg-synthetic-visual="1"\]\{display:block!important;width:100%!important;max-width:none!important/, 'synthetic plates must re-assert one-canvas geometry above every theme rule in Continuous mode');
assert.match(visualCache, /document\.body\?\.classList\?\.contains\("reader-flow-scrolled"\)/, 'synthetic plate immunity must be scoped to the Continuous flow; Paginated keeps its inline frame controller');
assert.match(mediaWidthSpec, /Continuous Reader keeps artwork independent of the text-width setting/, 'the v2.6.7 real-browser regression must remain in the E2E suite');
assert.match(mediaWidthSpec, /must not change width with the text-width setting/, 'the regression must pin artwork geometry across text-width changes');
assert.match(mediaWidthSpec, /the text-width setting must still shape prose/, 'the regression must prove prose still follows the text-width setting');
...
assert.match(rootReadme, /^# Shadow Garden v2\.6\.[0-7]/m);
...
console.log('v2.6/v2.6.7 release, real-browser, Library, Reader, Keeper, and accessibility contracts OK');
