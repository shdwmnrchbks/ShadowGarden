...
for(const marker of ['../domain/progress.js','../domain/bookmarks.js','canonicalIdentity','writeProgressAliases'])if(!readerStorage.includes(marker))fail(`Reader storage is missing ${marker}`);
if(readerStorage.includes('localStorage.'))fail("Reader storage must not bypass R2 services");

for(const marker of ["volume-finished-toggle","data-sg-finished-toggle","Mark as Finished","ShadowGardenBookAccess?.initial","ShadowGardenData?.loadCatalog","status.volumeAliases","setAliasesFinished","document.addEventListener(\"change\"","document.addEventListener(\"click\"","volume-complete-next","persist(true,{quiet:true})","volume-end-page-continuous"])if(!readerFinished.includes(marker))fail(`Reader finished control is missing ${marker}`);
...
if(readerFinished.includes('toggle.addEventListener("change"'))fail("Finished persistence must remain delegated for the Continuous clone");
for(const marker of ["window.__sgReaderPublicBookId","window.__sgReaderSourcePath","restartRequested","resetForReadAgain","readingState.clearProgressAliases","catalog.findVolumeEntry","clearRestartFlag"])if(!readerBootstrap.includes(marker))fail(`Reader bootstrap is missing ${marker}`);
if(readerBootstrap.includes("setInterval(sync,500)"))fail("obsolete Reader polling mirror returned");
if(!readerHtml.includes("reader-bootstrap.js"))fail("Reader HTML must load Reader bootstrap");
...
for(const marker of ["/assets/js/domain/*","/assets/js/reading-status.js","/assets/js/reader.js","/assets/js/reader-bootstrap.js","/assets/js/series.js","/assets/js/series-renderers.js","/assets/js/library-renderers.js","/assets/js/public/volume-actions.js"])if(!headers.includes(marker))fail(`fresh-cache headers are missing ${marker}`);
