...
for(const marker of ["mode:\"pinch\"","mode:\"pan\"","mode:\"swipe\"","lastTap","event.ctrlKey||event.metaKey","reader-zoomed","scale:()=>state.scale","touchstart","touchmove","touchend"])if(!readerGestures.includes(marker))fail(`Reader unified gesture/zoom controller is missing ${marker}`);
if(!readerHtml.includes("reader-bootstrap.js")||!readerHtml.includes("reader-zoom.css")||!readerHtml.includes('id="zoomViewport"')||!readerHtml.includes('id="zoomIn"')||!readerHtml.includes('id="zoomReset"'))fail("Reader HTML must load the R4 app/zoom surface");
