...
const [pkgText,roadmap,docsIndex,architectureIndex,readerDoc,readerHtml,bootstrap,session,app,storage,settings,progress,bookmarks,completion,gestures,rendition,paginated,continuous,pageMap,visualCache,continuousCore,zoomCss,headers,legacyText,manifestText]=await Promise.all([
  read("package.json"),read("docs/roadmaps/REFACTOR_ROADMAP.md"),read("docs/README.md"),read("docs/architecture/README.md"),read("docs/architecture/READER_LAYER.md"),read("src/reader.html"),read("src/assets/js/reader-bootstrap.js"),read("src/assets/js/reader/book-session.js"),read("src/assets/js/reader/app.js"),read("src/assets/js/reader/storage.js"),read("src/assets/js/reader/settings.js"),read("src/assets/js/reader/progress-controller.js"),read("src/assets/js/reader/bookmarks-controller.js"),read("src/assets/js/reader/completion.js"),read("src/assets/js/reader/gestures.js"),read("src/assets/js/reader/rendition.js"),read("src/assets/js/reader/paginated.js"),read("src/assets/js/reader/continuous.js"),read("src/assets/js/reader/page-map.js"),read("src/assets/js/reader-visual-cache.js"),read("src/assets/js/reader-continuous-core.js"),read("src/assets/css/reader-zoom.css"),read("src/_headers"),read("docs/architecture/r1-legacy-source-exceptions.json"),read("docs/architecture/v1-entrypoints.json")
]);
...
for(const file of retired){
  if(await exists(file))fail(`retired R4 Reader owner returned: ${file}`);
  if(!(legacy.removedDeadFiles||[]).includes(file))fail(`R1 dead-file manifest must remember R4 retirement: ${file}`);
}
...
if(!session.includes('throw new Error("Shadow Garden could not reset this volume. Your reading place was left unchanged.")'))fail("Read Again startup reset must fail closed when local state cannot be verified clear");
...
if(completion.includes("localStorage."))fail("Reader completion state must remain R2-domain owned");

for(const marker of ['mode:"pinch"','mode:"pan"','mode:"swipe"','lastTap','event.ctrlKey||event.metaKey','reader-zoomed','touchstart','touchmove','touchend','isVisualDocument','?4:3','zoomAt(2.2','scale:()=>state.scale'])if(!gestures.includes(marker))fail(`unified Reader gesture/zoom controller is missing ${marker}`);
if(gestures.includes("MutationObserver")||gestures.includes("window.ePub=")||gestures.includes("rendition.resize")||gestures.includes("pageMap"))fail("Reader zoom must not regain iframe observation/ePub wrapping or mutate rendition/Page Map layout");
for(const marker of [".reader-zoom-viewport",".reader-zoom-layer","transform-origin:0 0","body.reader-zoomed .page-hit","reader-zoom-controls"]){if(!zoomCss.includes(marker))fail(`Reader zoom CSS is missing ${marker}`)}
if(!readerHtml.includes('<div id="zoomViewport" class="reader-zoom-viewport">')||!readerHtml.includes('<div id="zoomLayer" class="reader-zoom-layer"><div id="viewer" class="viewer"></div></div>'))fail("EPUB viewer must live inside the outer R4 zoom transform layer");
for(const marker of ['id="zoomIn"','id="zoomOut"','id="zoomReset"','id="zoomValue"','Pinch or double-tap'])if(!readerHtml.includes(marker))fail(`Reader zoom UI is missing ${marker}`);
...
for(const marker of ["/assets/js/reader-bootstrap.js","/assets/js/reader-a11y.js","/assets/js/reader-continuous-core.js","/assets/js/reader-visual-cache.js"]){if(!readerHtml.includes(marker))fail(`Reader HTML is missing retained entry/compatibility asset ${marker}`)}
if(!readerHtml.includes("/assets/css/reader-zoom.css")||!readerHtml.includes("/assets/css/reading-status.css"))fail("Reader must load zoom and Finished-state presentation directly");
...
for(const marker of ["READER_LAYER.md","R4 Reader"]){if(!docsIndex.includes(marker)&&!architectureIndex.includes(marker))fail(`documentation indexes must include ${marker}`)}
for(const marker of ["Reader Application Layer","Authorized book session","Zoom contract","Retired Reader ownership scripts","session-only visual viewport transform"]){if(!readerDoc.includes(marker))fail(`Reader architecture document is missing ${marker}`)}
if(!roadmap.includes("R4. Reader architecture refactor | ✅ Done")||!roadmap.includes("Release:** v1.18.0"))fail("Refactor roadmap must record R4 complete as v1.18.0");
...
try{
  ...
  for(const alias of [source,publicId]){
    const item=JSON.parse(localStorage.getItem(`sg-progress:${alias}`)||"null");
    if(item?.file!==publicId||item?.page!==7)fail(`Reader progress alias write failed for ${alias}`);
  }
  ...
  const sanitized=settingsModule.sanitizeReaderSettings({fontSize:999,lineHeight:.1,width:1,flow:"bad",theme:"bad",swipeTurns:false});
...
if(failures.length){
  console.error(`Shadow Garden R4 Reader architecture/zoom check failed with ${failures.length} problem${failures.length===1?"":"s"}:`);
  failures.forEach(message=>console.error(`- ${message}`));
  process.exitCode=1;
}else console.log("Shadow Garden R4 explicit Reader ownership, canonical state handoff, and viewport zoom contracts passed.");
