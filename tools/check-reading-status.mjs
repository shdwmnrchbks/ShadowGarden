...
const [facade,stateSource,progressSource,readerStorage,readerCompletion,readerSession,readerApp,readerGestures,readerBootstrap,continuousCore,series,seriesRenderers,volumeActions,volumeActionsStyle,library,libraryRenderers,style,compactStyle,footerVersion,writeSource,adminBootstrap,adminVersion,headers,pkg,readerHtml,indexHtml,adultHtml,seriesHtml]=await Promise.all([
  read("src/assets/js/reading-status.js"),read("src/assets/js/domain/reading-state.js"),read("src/assets/js/domain/progress.js"),read("src/assets/js/reader/storage.js"),read("src/assets/js/reader/completion.js"),read("src/assets/js/reader/book-session.js"),read("src/assets/js/reader/app.js"),read("src/assets/js/reader/gestures.js"),read("src/assets/js/reader-bootstrap.js"),read("src/assets/js/reader-continuous-core.js"),read("src/assets/js/series.js"),read("src/assets/js/series-renderers.js"),read("src/assets/js/public/volume-actions.js"),read("src/assets/css/volume-actions.css"),read("src/assets/js/library.js"),read("src/assets/js/library-renderers.js"),read("src/assets/css/reading-status.css"),read("src/assets/css/library-compact-alignment.css"),read("src/assets/js/library-footer-version.js"),read("tools/write-source.mjs"),read("src/assets/js/admin-bootstrap.js"),read("src/assets/css/admin-version.css"),read("src/_headers"),read("package.json"),read("src/reader.html"),read("src/index.html"),read("src/nsfw.html"),read("src/series.html")
]);
...
for(const marker of ["createReaderStorage","createProgressController","createBookmarksController","createGestureController","createCompletionController","createPageMapController","session.sourcePath","session.publicBookId||session.storageIdentity","imageFocus:elements.imageFocus"])if(!readerApp.includes(marker))fail(`Reader app orchestrator is missing ${marker}`);
for(const marker of ["beginSwipe","finishSwipe","openImageFocus","closeImageFocus","isImageFocused","touchstart","touchend"])if(!readerGestures.includes(marker))fail(`Reader navigation/image-focus controller is missing ${marker}`);
if(readerGestures.includes('doc.addEventListener("touchmove"')||readerGestures.includes("reader-zoomed")||readerGestures.includes("zoomViewport"))fail("Reader must not restore page-wide touchmove/zoom interception");
if(!readerHtml.includes("reader-bootstrap.js")||!readerHtml.includes("reader-zoom.css")||!readerHtml.includes('id="imageFocus"')||!readerHtml.includes('id="imageFocusViewport"')||!readerHtml.includes('<div id="viewer" class="viewer"></div>'))fail("Reader HTML must load the R4 app plus isolated image-focus surface");
for(const retired of ["reader-polish.js","reader-v1.10.1.js","reader-gesture-hook.js","reader-wheel-pages.js","reader-finished.js"])if(readerHtml.includes(retired))fail(`retired Reader controller is still loaded: ${retired}`);
...
for(const marker of ["/assets/js/domain/*","/assets/js/reader/*","/assets/js/reader-bootstrap.js","/assets/js/series.js","/assets/js/series-renderers.js","/assets/js/library-renderers.js","/assets/js/public/volume-actions.js","/assets/css/reader-zoom.css"])if(!headers.includes(marker))fail(`fresh-cache headers are missing ${marker}`);
